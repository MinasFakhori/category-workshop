...
public class CustomerView implements Observer
{
  ...
  private final JButton theAllBTN = new JButton(Name.DISPLAYALL);


  public void setBtnCheck(boolean b) {
    if (b) {
      theBtCheck.setBackground(Color.GREEN);
    } else {
      theBtCheck.setBackground(Color.RED);
    }
  }
  ...
  public CustomerView( RootPaneContainer rpc, MiddleFactory mf, int x, int y )
  {
    ...
    theAction.setBounds( 120, 25 , 270, 20 );       // Message area
    theAction.setText( "" );                        //  Blank
    cp.add( theAction );                            //  Add to canvas

    theInput.setBounds( 120, 50, 270, 40 );         // Product no area
    theInput.setText("");                           // Blank
    cp.add( theInput );                             //  Add to canvas
    
    theSP.setBounds( 120, 100, 270, 160 );          // Scrolling pane
